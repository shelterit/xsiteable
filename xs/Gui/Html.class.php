<?php
    ...
    class Html {
        ...
        function _render_headers ( $keys, $conf = array (), $mode = self::RENDER_HTML ) {
            $ret = null ;
            if ( $mode == self::RENDER_HTML ) $ret .= "<thead><tr>\n";
            if ( $mode == self::RENDER_HTML ) $ret .= "<th>No.</th>\n";
            foreach ( $keys as $key ) {
                if ( isset ( $conf[$key] ) ) {
                    if ( $conf[$key] !== false ) {
                        if ( strpos($key, 'flag:') !== false )
                           $key = str_replace ('flag:', '', $key ) ;
                        if ( $mode == self::RENDER_HTML ) $ret .= "<th>$key</th>\n";
                        else $ret[$key] = $key ;
                    }
                } else {
                   if ( $mode == self::RENDER_HTML ) $ret .= "<th>$key</th>\n";
            ...
            if ( $mode == self::RENDER_HTML ) $ret .= "</tr></thead>\n";
            return $ret ;
        }

        function _render_row ( $keys, $conf = array (), $mode = self::RENDER_HTML ) {
            
            $ret = null ;

            if ( $mode == self::RENDER_HTML ) $ret .= "<tr>\n";
            if ( $mode == self::RENDER_HTML ) $ret .= "<td> $this->counter</td>\n"; // (".print_r(array_keys($keys),true).")
                ...
                if ( isset ( $conf[$key] ) ) {
                    ...
                    if ( $conf[$key] !== false ) {
                        ...
                        if ( strpos ( $key, 'flag:' ) !== false ) {
                            ...
                            if ( $mode == self::RENDER_HTML ) $ret .= '<td class="color-'.$cls.'" style="'.$stl.'"> '.$value.' </td>' ;
                            else $ret[$key] = $value ;
                        ...
                        // if ( $mode == self::RENDER_HTML ) $ret .= "<td>$value ({$conf[$key]})</td>" ;
                    }

                } else {
                    if ( $mode == self::RENDER_HTML ) $ret .= "<td>$value</td>" ;
                    else $ret[$key] = $value ;
                }
            }
        ...
        function _render_table ( $table, $conf = array (), $mode = self::RENDER_HTML ) {
...
// echo "[".count($table)."]" ;
            ...
            if ( $mode == self::RENDER_HTML ) $ret .= "</tbody></table>\n";

            switch ( $mode ) {
                case self::RENDER_EXCEL :
                case self::RENDER_CSV   : 
                case self::RENDER_JSON  :
                case self::RETURN_DATA  : return $data ; break ;
                case self::RENDER_HTML  : return $ret ; break ;
            }
        ...
        function _render_csv_table ( $table, $conf = array () ) {
            $this->_render_table ( $table, $conf, self::RENDER_CSV ) ;
        }

        function _render_excel_table ( $table, $conf = array () ) {
            $this->_render_table ( $table, $conf, self::RENDER_EXCEL ) ;
        }

        function _render_json_table ( $table, $conf = array () ) {
            $this->_render_table ( $table, $conf, self::RENDER_JSON ) ;
        }

        function _render_xml_table ( $table, $conf = array () ) {
            $this->_render_table ( $table, $conf, self::RENDER_XML ) ;
        }
