<?php
    ...
    class User extends \xs\Events\Plugin {
        ...
        function ___on_user_config () {
            ...
            if ( $this->debug ) debug('xs_User :: on_user_config()') ;
            ...
            if ( isset ( $this->glob->config['dev_user_management']['DEV'] ) &&
                ...
                // if ( $this->debug ) debug('on_user_config()') ;
        ...
        function callback___on_user_config ( $param = null ) {
            ...
            if ( $this->debug ) debug('callback___on_user_config()','FOUND PARAMS') ;
                
            } else {
            if ( $this->debug ) debug('callback___on_user_config()','NO PARAMS') ;
        ...
        function callback_final___on_user_config () {
            ...
            // if ( $this->debug ) debug_r($this->credentials) ;
        }
        ...
        function database_sync () {
            ...
            if ( ! $find ) {
                ...
                $t = @isset ( $this->_type->_user ) ? $this->_type->_user : 41 ;
                ...
                $this->alert ( 'comment', 'Just a note ...', '... that your user profile has been first-time synhronized.' ) ;
        ...
        function isAllowed ( $func, $default ) {
            
            // get security!
            $sec = $this->_get_module ( 'security' ) ;
            
            // just check if we're an admin; if so, free rides for all!
            if ( $this->inList('*', 'role') === null )
                return true ;

            // just return access evaluation
            $test = $sec->has_access ( $func, $default ) ;
            // debug($test,'ffffffffffffffffff');
            return $test ;
        }
        ...
        function isUsertype ( $type ) {
            ...
            // return false;
            $role = '' ;
            // debug_r ( $_SESSION['session_user_credentials'] ) ;
            if ( isset ( $_SESSION['session_user_credentials'] ) && $_SESSION['session_user_credentials'] )
                $role = '@user' ;
            else
                $role = '@anonymous' ;
            if ( $type == '@all' ) 
        ...
        function isRole ( $find ) {
            return $this->inList ( $find, 'role' ) ;
        }
        ...
        function inGroup ( $find ) {
            return $this->inList ( $find, 'group' ) ;
        }
        
        function inList ( $find, $in = 'group' ) {
            
            if ( ! is_array ( $find ) )
                $find = explode ( '|', $find ) ;

            if ( isset ( $this->values[$in] ) && $this->values[$in] ) {

                $list = $this->values[$in] ;

                // debug_r($list);
                if ( ! is_array ( $list ) )
                    $list = explode ( '|', $this->values[$in] ) ;
                ...
                foreach ( $list as $list_item ) {
                    ...
                    foreach ( $find as $item ) {
                        ...
                        if ( trim(strtoupper($list_item)) == trim(strtoupper($item)) ) {
                            // echo "TRUE " ;
                            return true ;
                        }
                    }
