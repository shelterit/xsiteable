...
function debug ( $data, $title = null ) {
    if ( $title ) 
        ...
        echo "<div style='float:right;margin-top:2px;margin-right:6px;margin-left:18px;padding:1px 3px;background-color:#deffde;font-size:0.7em;color:#333;'>".  debugPrintCallingFunction('#ed0', true)."</div>" ;
    echo '<pre style="clar:both;padding:4px;margin:4px;border:dotted 1px #999;color:#555;">' ;
    var_dump ( $data ) ;
    echo '</pre>' ;
}
...
function debugPrintCallingFunction ( $bkg = '#eee', $just_info = false ) { 
    $fi = 'n/a'; 
    $fu = 'n/a'; 
    $li = 'n/a'; 
    $debugTrace = debug_backtrace(); 
    $ret = "<pre style='padding:0;margin:3px 0 3px 15px;color:#666;background-color:{$bkg};'>"; 
    $r = '' ;
    foreach ( $debugTrace as $count => $trace ) {
        if ( $just_info && (int) $count > 4 ) break ;
        if ( (int) $count == 0 ) continue ;
        $fi = isset ( $trace['file'] ) ? $trace['file'] : 'n/a' ;
        $fu = isset ( $trace['function'] ) ? $trace['function'] : 'n/a' ;
        $li = isset ( $trace['line'] ) ? $trace['line'] : 'n/a' ;
        $ret .= "[{$count}] file [{$fi}], function [{$fu}], line [{$li}]\n"; 
        $r .= "{$count}:$fi :: $fu|<br/>" ;
    }
    $ret .= "</pre>"; 
    if ( $just_info ) return $r ;
    return $ret ;
} 
    ...
    function timed_mili ( $mili ) {
        // $seconds = $mili / 1000;
        $d = date ( 'Y-m-d H:i:s', $mili ) ;
        // debug_r ( $d ) ;
        return timed ( $d ) ;
    }
    ...
    function fetch_widget_setup_xml ( $name ) {
        ...
        if ( trim ( $name ) == '' )
            $name = $xs_stack->glob->request->_controller_name ;
        
        $widget = $xs_stack->get ( $name ) ;
// debug_r($widget);
        $src = $widget->gui_setup () ;

        // debug_r(htmlentities($src),$name) ;
...
function security_user_check_function ( $func, $default = null ) {
    global $xs_stack ;
    if ( is_array ( $func ) )
        $func = end ( $func ) ;
    if ( is_array ( $default ) )
    ...
    $default_value = null ;
    if ( trim ( (string) $default ) == 'deny' )
        $default_value = false ;
    if ( trim ( (string) $default ) == 'allow' )
        $default_value = true ;
    // if ( $default == null ) $default_value = true ;
    
        // $default_value = true ;
    // debug($default,$func);
    // debug($default,'core :: '.$func);
    // debug($default_value,'core :: '.$func);
    return $xs_stack->glob->user->isAllowed ( $func, $default_value ) ;
    // if ( $t ) return '1' ;
    // return '0' ;
}
...
*/
function widget ( $controller_name = 'not given', $function = 'no what', $name = null, $param = null, $id=null, $content=null ) {
    
    global $xs_stack ;
    
    if ( $id !== null ) {
        $widget = $xs_stack->get_instance ( $id ) ;
        // debug_r ( $widget ) ;
    } else {
        $widget = $xs_stack->get ( $controller_name ) ;
    }
    ...
    if ( $widget == null )
        ...
        // debug($function);
        $t = $widget->$function ( $name, $param, $id ) ;
        // debug($t);
        if ( $t )
            return $t ;
    }
...
function widget_properties ( $widget, $id ) {
    ...
    return _dom_array ( $instance->get_properties ( $id ) ) ; // _properties->__getArray () ) ;
}
...
function widget_settings ( $widget, $id ) {
    ...
    if ( ! $instance ) return null ;
    // $instance = $xs_stack->get ( $widget ) ;
    return _dom_array ( $instance->get_settings ( $id ) ) ; // ->__getArray () ) ;
}
...
function widget_property ( $widget, $property, $id ) {
    ...
    // return _dom_array_item ( $property,  ) ; // properties->__getArray () ) ;
}

function widget_setting ( $widget, $setting, $id ) {
    global $xs_stack ;
    $instance = $xs_stack->get_instance ( $id ) ;
    if ( ! $instance ) return null ;
    // debug_r($instance, $id );
    $v = $instance->get_setting ( $setting ) ;
    // debug_r ($instance, "[$widget]-[$setting]-[$id] " ) ;
    // debug_r ($instance, "[$widget]-[$setting]-[$id] " ) ;
    return $v ;
    // return _dom_array_item ( $setting, $v ) ;// >__getArray () ) ;
}
...
function natsort2d( &$arrIn, $index = null, $dir = 'asc' )
{
