<?php
    ...
    class xs_module_core extends \xs\Events\Module {
        ...
        protected $___register_types = array ( 
            ...
            // basic standard topic types
            '_page' => 'A page',
            '_widget' => 'A widget',
            '_function' => 'A function',
            
            '_user' => 'A user',
            '_user_group' => 'A user group',
            '_user_role' => 'A user role',
            '_user_function' => 'A user function',
            
            '_container' => 'A container of objects',
            
            '_blog' => 'A blog',
            '_blog_item' => 'A blog item',
            
            '_news' => 'A news point',
            '_news_item' => 'A news item',
            
            '_forum' => 'A forum point',
            '_forum_item' => 'A forum item',
            
            '_comment' => 'A comment',
            
        ) ;
        
        // define some aliases for earlier versions of xSiteable (slated 
        // for deletion after known sites are converted)
        ...
        public $resource_source_id = null ;
        ...
        function ___modules ( $uri = null ) {
            ...
            // Btw, they're also registered fairly late to give other plugins a chance
            // to register before core.module kicks in
       ...
       function ___env () {
        ...
        function on_request_db ( $uri = null ) {
            ...
            if ( count ( $this->page_db_lookup ) > 0 ) {
                ...
                if ( isset ( $this->page_db_lookup[$pid]['_widget_index'] ) )
                    $this->glob->page->_widget_index = $this->page_db_lookup[$pid]['_widget_index'] ;
                
                // create resource, and return it
                $page_type = isset ( $p['page-type'] ) ? $p['page-type'] : null ;
                $this->glob->page->page_type = $page_type ;
...
*/
        ...
        function ___output_pre () {
            ...
            if ( isset ( $_SESSION['xs_alerts'] ) ) {
                
                // fetch the thing
                $v = $_SESSION['xs_alerts'] ;
                
                // is it array or string?
                if ( is_string ( $v ) )
                    $v = (array) @unserialize ( $_SESSION['xs_alerts'] ) ;
                
                $this->glob->alerts = array_merge ( $this->glob->alerts, $v ) ;
                // debug ( $this->glob->alerts ) ;
                unset ( $_SESSION['xs_alerts'] ) ;
            }
        ...
        function ___register_functionality () {
            $this->_register_functionality ( 'Page view', 'page', null ) ;
            $this->_register_functionality ( 'Page functions', 'page:*', 'a=role:editor' ) ;
            $this->_register_functionality ( 'Admin *', 'admin:*', 'all' ) ;
            $this->_register_functionality ( 'Admin bar', 'admin:bar', 'd=usertype:all|a=role:admin' ) ;
            $this->_register_functionality ( 'Admin page', 'admin:bar:page' ) ;
            $this->_register_functionality ( 'Admin access', 'admin:bar:access' ) ;
            $this->_register_functionality ( 'Admin widgets', 'admin:bar:widgets' ) ;
        }
