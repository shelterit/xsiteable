<?php
    ...
    class DataManager extends \xs\Core {
        ...
        function get_adapter ( $adapter_id = 'tmp' ) {
            ...
            if ( ! isset ( $this->db[$adapter_id] ) ) {
                ...
                try {
                    ...
                    $this->db[$adapter_id]['class'] = "\xs\Data\Adapter\{$adapter_id}" ;
        ...
        public function register_datasource ( $datasource_name, $adapter = 'pdo', $config = array () ) {

            $class = '\\xs\\Data\\Adapter\\' . $adapter ;

            // register that our datasource uses a particular adapter
            if ( ! isset ( $this->db[$datasource_name] ) ) {
                $this->db[$datasource_name]['adapter'] = $adapter;
                $this->db[$datasource_name]['config'] = $config ;
                $this->db[$datasource_name]['instance'] = null ;
                $this->db[$datasource_name]['class'] = $class ;
                if ( $this->debug ) $this->glob->seclog->logInfo ( '['.$this->glob->user->username."] dataManager->register_datasource ($datasource_name, $adapter ) Ok."  ) ;
            } else {
                if ( $this->debug ) $this->glob->seclog->logInfo ( '['.$this->glob->user->username."] dataManager->register_datasource ($datasource_name, $adapter ) already registered."  ) ;
            }
        ...
        public function register_query ( $database, $token, $sql, $timer = XS_CACHE_DEFAULT, $params = array () ) {
            ...
            */
            ...
            if ( ! isset ( $this->query[$token] ) ) {
                ...
                if ( isset ( $this->glob->config['caching'][$token] ) ) {
                   $timer = $this->glob->config['caching'][$token] ;
                   // debug($timer);
                }
                ...
                $this->query[$token]['cache'] = new \xs\Cache\Cache (
                    $this->query[$token]['id'],
        ...
        public function get_cache ( $token ) {
        ...
        public function reset ( $token ) {
            ...
            // debug_r ( array_keys($this->query), 'reset' ) ;
            ...
            if ( $item ) {
                ...
                $filename = $this->glob->config['framework']['cache_directory'].'/'.$token.'.tmp' ;
