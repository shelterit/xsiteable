...
class adLDAP {
    ...
    */
    ...
    /**
    * Control max depth of recursion when looking up groups. -1 means disabled.
    *
    * @var int
    */
    protected $_max_recursion = 4 ;
    ...
    * @return bool
    */
    function __construct($options=array(), $log = null ){
    ...
    */
    public function connect() {
            // $this->log->logInfo ( '['.$_SERVER['REMOTE_ADDR'].'] {adldap:connect} ' ) ;
        ...
        if ($this->_base_dn == NULL) {
            ...
            // $this->log->logInfo ( '['.$_SERVER['REMOTE_ADDR'].'] {adldap:connect/end} ' ) ;
        return (true);
    }
    ...
    * @return array
    */
    public function recursive_groups($group, $depth = -1 ){
        global $beans ;
        ...
        if ( $this->_max_recursion != -1 )
            $depth++ ;

        if ( $depth > $this->_max_recursion )
            return $ret_groups ;
        
        if ( $this->debug ) echo "<div style='margin:5px 15px;padding:5px;border:solid 1px #bbb;'> $group ($depth) " ;
        ...
        if (isset($groups[0]["memberof"]) && is_array($groups[0]["memberof"])) {
            ...
            if ($groups){
                ...
                foreach ($group_names as $id => $group_name){
                    $child_groups=$this->recursive_groups($group_name, $depth);
