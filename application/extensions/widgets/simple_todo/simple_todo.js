$(document).ready(function(){
		...
		update		: function(){		// The function is called after the todos are rearranged
			...
			arr = $.map(arr,function(val,key){
				return val.replace('todo-','');
                        ...
                        var widget = $(this).parent().parent().parent().attr('id') ;

			// Saving with AJAX
			$.get(xs_dir.api + '/widgets/simple_todo',{widget: widget, action:'rearrange',positions:arr}, function (res) {
                            $("#callbacks").html(res) ;
                        });
		},
	...
	$("#dialog-confirm").dialog({
		...
		buttons: {
			'Delete item': function() {
                                ...
                                // var widget = currentTODO.parent().parent().parent().attr('id') ; alert ( widget ) ;

				$.get(xs_dir.api + 'widgets/simple_todo',{"action":"delete","id":currentTODO.data('id')},function(res){
	...
	$('.todo a.saveChanges').live('click',function(){
		...
		$.get(xs_dir.api + '/widgets/simple_todo',{'action':'edit','id':currentTODO.data('id'),'text':text}, function(res) {
                            $("#callbacks").html(res) ;
                });
	...
	$('#addButton').click(function(e){
		...
		$.get(xs_dir.home + '/_api/widgets/simple_todo',{'action':'new','text':'New Todo Item.','rand':Math.random()},function(msg){
