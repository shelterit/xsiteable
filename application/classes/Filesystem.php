...
class Filesystem {
    ...
    static function fileWriteArray($file, $arr) {
        return ( $this->fileWrite($file, implode("\r\n", $arr)) );
    }

    // Write file

    static function fileWrite($file, $data) {
    ...
    static function fileReadArray($file) {
        ...
        while (true) {
            $buffer = @file($file);
            if ($buffer || $attempt++ == Filesystem::attempts)
                break;
        }
    ...
    static function filePropertiesRead($file, $options = false) {

        $in = $this->fileReadArray($file);
